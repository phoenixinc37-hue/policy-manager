...
export default function ManagerDashboardPage() {
  ...
  const snapshot = getManagerSnapshot(policies, acknowledgments);
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <PageHeader
        ...
        description="This now reads more like a true beta operator console: live rollout health, clinic pressure points, people to chase, and draft readiness in one place."
      ...
      <div className="mb-8 grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <section className="card">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2>Draft queue</h2>
              <p className="mt-1 text-sm text-slate-500">What a practice manager would review before controlled publish.</p>
            </div>
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{snapshot.drafts.length} open</span>
          </div>
              ...
              <Link key={draft.id} href={`/policy/${draft.id}/edit`} className="block rounded-2xl border border-slate-200 p-4 transition hover:border-cyan-200 hover:bg-cyan-50/40">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">Effective {formatDate(draft.effectiveDate)}</p>
                  </div>
        ...
        <section className="card">
          ...
          <div className="space-y-3">
            ...
            {policyRollout.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-200 px-4 py-6 text-sm text-slate-500">No published documents with acknowledgments match this clinic filter yet.</div> : null}
          </div>
        </section>
      </div>
      ...
      <div className="mb-8 grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
        <section className="card">
          <div className="mb-4">
            <h2>Top follow-up list</h2>
            <p className="mt-1 text-sm text-slate-500">The most sales-ready manager panel in the app: exactly who needs a nudge next.</p>
          </div>
          <div className="space-y-3">
            {snapshot.topFollowUps.map((item) => (
              <div key={item.id} className="rounded-2xl border border-rose-200 bg-rose-50/70 p-4">
                ...
                <p className="mt-1 text-sm text-slate-500">{item.clinicName}</p>
                <p className="mt-2 text-sm text-slate-700">Still outstanding: {item.policyTitle}</p>
                <p className="mt-1 text-sm text-rose-700">Due {formatDate(item.dueDate)} · {dueLabel(item.dueDate)}</p>
              </div>
            ))}
        ...
        <section className="card">
          <div className="mb-4">
            <h2>Clinic rollout heatmap</h2>
            <p className="mt-1 text-sm text-slate-500">Gives a manager quick visibility into which site is smooth and which site is sticky.</p>
          </div>
          <div className="space-y-3">
            {snapshot.clinicStats.map((item) => (
              <div key={item.clinic.id} className="rounded-2xl border border-slate-200 p-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="font-semibold text-slate-900">{item.clinic.name}</p>
                    <p className="mt-1 text-sm text-slate-500">{item.clinicPolicies} live docs · {item.clinicPending} pending acknowledgments</p>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-semibold">
                    <span className="rounded-full bg-slate-100 px-2.5 py-1 text-slate-600">{item.rate}% complete</span>
                    {item.clinicOverdue > 0 ? <span className="rounded-full bg-rose-100 px-2.5 py-1 text-rose-700">{item.clinicOverdue} overdue</span> : <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-emerald-700">On track</span>}
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="card overflow-x-auto">
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-slate-500">Strong enough for sales because it shows who a manager would actually chase and where operational friction sits.</p>
        </div>

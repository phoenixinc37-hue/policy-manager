...
export default function CompletionPage({ searchParams }: { searchParams?: { days?: string } }) {
  const [completedDocs, setCompletedDocs] = useState<SavedDraftRecord[]>([]);

  useEffect(() => {
    function forceLoad() {
      setCompletedDocs(loadLibraryDrafts());
    }
    forceLoad();
  ...
  const completed = completedDocs.filter((doc) => {
    const timestamp = doc.completedAt || doc.savedAt;
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 16px 48px" }}>
        <header style={headerCard}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <CabinetGraphic />
            <div>
              <div style={{ fontSize: 20, fontWeight: 700 }}>Policy Manager <span style={{ fontSize: 20, fontWeight: 700, color: "#dc2626", marginLeft: 10 }}>· Firm Completion</span></div>
          ...
          <section style={panelCard}>
            ...
            <div style={{ border: "1px solid #e2e8f0", borderRadius: 12, overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                {completed.length === 0 ? (
                  <div style={{ padding: "24px 20px", color: "#64748b", fontSize: 14 }}>No documents have been archived in this time range.</div>
                ) : completed.map((doc, idx) => (
                  <div key={doc.id} style={{ ...tableRow, borderBottom: idx === completed.length - 1 ? "none" : "1px solid #e2e8f0" }}>
                    <div>
                      {doc.documentType === "Policy" && <span style={{ ...typeBadge, background: "#dcfce7", color: "#166534" }}>POLICY</span>}
                      {doc.documentType === "SOG" && <span style={{ ...typeBadge, background: "#e0f2fe", color: "#0369a1" }}>SOG</span>}
                      {doc.documentType === "Memo" && <span style={{ ...typeBadge, background: "#f3f4f6", color: "#475569" }}>MEMO</span>}
                    </div>
                    ...
                    <div style={{ fontSize: 13, color: "#64748b" }}>
                      {doc.circulationTargets ? doc.circulationTargets.join(", ") : "None"}
                    </div>
                    <div style={{ fontSize: 13, color: "#1f7a37", fontWeight: 700 }}>
                      {new Date(doc.completedAt || doc.savedAt).toLocaleDateString()}
                    </div>

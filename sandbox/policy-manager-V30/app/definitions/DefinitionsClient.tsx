...
export function DefinitionsClient({ initialConfig, initialDefinitions }: { initialConfig: any, initialDefinitions: any[] }) {
  const [config, setConfig] = useState(initialConfig);
  const [isPending, startTransition] = useTransition();

  const toggleInclude = () => {
    const newValue = config.includeCoreDefinitions === "Yes" ? "No" : "Yes";
    setConfig({ ...config, includeCoreDefinitions: newValue });
    startTransition(() => {
      const formData = new FormData();
      formData.append("includeCoreDefinitions", newValue);

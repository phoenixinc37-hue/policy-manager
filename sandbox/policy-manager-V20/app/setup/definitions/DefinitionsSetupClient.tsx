...
import { saveDefinitionAction, saveConfigAction } from "../../actions";

export function DefinitionsSetupClient({ initialDefinitions, initialMode }: { initialDefinitions: any[], initialMode: string }) {
  const [draftDefinitions, setDraftDefinitions] = useState(initialDefinitions);
  ...
  const saveDefinitions = () => {
    startTransition(async () => {
      ...
      for (const def of draftDefinitions) {
        await saveDefinitionAction(def.key, def.label, def.definition);
      }
      
      const formData = new FormData();
      formData.append("includeCoreDefinitions", "Yes");
      formData.append("definitionMode", mode);
      await saveConfigAction(formData);
    });
  };

  const resetDefinitions = () => {
    // not implementing full reset since we don't have defaultDefinitions easily available,
    // but we can just reload or do nothing
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "24px 16px 48px" }}>
        <header style={headerCard}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <CabinetGraphic />
            <div>
              <div style={{ fontSize: 20, fontWeight: 700 }}>Policy Manager V10 <span style={{ fontSize: 20, fontWeight: 700, color: "#2e7d32", marginLeft: 10 }}>· Definitions Setup</span></div>
        ...
        <main style={{ marginTop: 24, display: "grid", gap: 20 }}>
          ...
          <section style={panelCard}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 18 }}>
              <div style={modeCard(mode === "manual")}>
                ...
                <button type="button" onClick={() => setMode("manual")} style={mode === "manual" ? activeLinkButton : secondaryLinkButton}>Use manual mode</button>
              </div>
              ...
              <div style={modeCard(mode === "ai")}>
                ...
                <button type="button" onClick={() => setMode("ai")} style={mode === "ai" ? activeLinkButton : secondaryLinkButton}>Use AI mode</button>
              </div>
            </div>
          </section>
          ...
          <section style={panelCard}>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 18 }}>
              <button type="button" onClick={saveDefinitions} style={saveButton}>{isPending ? "Saving..." : "Save definitions to home page"}</button>

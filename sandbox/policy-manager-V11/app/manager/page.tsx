            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 14 }}>
              {statusCards.map((card) => (
                <div key={card.title} style={statusCard}>
                  ...
                  <Link href={card.button === "View circulation" ? "/status" : card.button === "View approvals" ? "/pending" : card.button === "View report" ? "/completion" : "/policy-index"} style={whiteWideButtonLink}>{card.button}</Link>
                </div>
              ))}
            </div>
          </section>

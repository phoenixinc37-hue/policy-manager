# FULL7S — V1.25 | March 31, 2026
*Complete Phoenix Inc structure — 7 Summits + Base Camp with Issues + Tasks*
*Code: FULL7S | Version: 1.24 | Confirmed working version*
*Previous: V1.23*

---

## The 7 Summits of Phoenix Inc

**S1 — Clinic Solutions Inc**

S1.0 — CSI (Company Level)
Issues:
S1.0.I1 — Governance: Tammy claiming operational authority without proper process. Overrode CEO instruction on Ross Friday — in writing.
S1.0.I2 — Accounting/Deb: Pushing back on personal legal expense draws from CSI. Pattern of operational overreach.
S1.0.I3 — Money Trail: Canmore sale proceeds allegedly used for supplier debt without authorization. Deb went to WDDC directly without Scott.
S1.0.I4 — Tammy email: attempting to delay implementation of Pricing Intelligence System.

S1.1 — Tudor Glen (TG) — No active issues
S1.2 — River Valley (RV) — No active issues
S1.3 — Rosslyn (ROSS) — No active issues
S1.4 — Urgent Pet Care (UPC-ROSS) — No active issues

Tasks:
S1.T1 — Canmore money trail
— S1.T1.1(a) *** — Scott: email sent to Kate with sale contract info ✅ Kate: file and process
— S1.T1.2(b) *** — Scott: email sent to Kate with proceeds info ✅ Kate: determine full amount sent to WLH
— S1.T1.3(c) — Kate: file sale contract in WLH folder

S1.T2 — WhatsApp messages
— S1.T2.2(d) — Kate: organize messages into timeline

S1.T3 — Deb / Legal expenses
— S1.T3.1(e) — Scott: send Blaise email with questions on WHL / personal taxes / Deb

S1.T4(f) **** — Scott: send 500 Day Plan email at 8:00 AM (Tue Mar 31) — Scott sending manually

---

**S2 — Wildeland Holdings Inc**

S2.0 — WLH (Company Level) — No active issues

S2.1 — Copperstone / Canmore
Issues:
S2.1.I1 — Unit 2104 sold — money trail active. Links to S1.T1, S1.T2.

S2.2 — Playa Del Sol / Kelowna
Issues:
S2.2.I1 — Special levy arrears — $8,423.16 owing. Amanda Johnston, Associated PM (amanda.johnston@associatedpm.ca / 250-712-0025).

Tasks:
S2.2.T1(ap) — Scott: look into details of special assessment (what is it for)
S2.2.T2(aq) — Deb: provide full Canmore proceeds accounting by EOD Thu Apr 2 — email sent ✅ (CC Tammy)
S2.2.T3(ar) — Scott: decide payment method once Canmore accounting received

S2.3 — Disney Vacation Club (DVC)
Issues:
S2.3.I1 — Rental income ~$4,000 sitting in PayPal — Tammy notified, no decision yet.

Tasks:
S2.T1 — PayPal / DVC funds
— S2.T1.1(g) PENDING — Email sent to ATB confirming bank info. Once received: set up PayPal account.
— S2.T1.2(h) PENDING — Waiting for bank info from ATB before transfer can proceed.

S2.T2 — Grand Californian holding points
— S2.T2.1(i) — COMPLETE — file ✅
— S2.T2.2(j) — Scott: confirm if info is correct on all 3 reservations — at least 1 is wrong. Call to update. DRIVING TASK.

---

**S3 — Lost Lake Resort Inc**

S3.0 — LLR (Company Level) — No active issues

S3.1 — Lake House, Jackfish Lake, Parkland County
Issues:
S3.1.I1 — Railing damaged — temp railing installed, permanent fix pending.
S3.1.I2 — TD Insurance claim active — TD Claim# 044795742. Stefan managing, Kyle on holiday.
S3.1.I3 — Starlink ordered — delivery pending.
S3.1.I4 — Router admin credentials still with Wayne — needs to change.

Tasks:
S3.T1 — Railing
— S3.T1.1(k) — COMPLETE ✅ — filed

S3.T5 — Art Weekend / Lost Lake Events
Notes:
- ASSC (Art Society of Strathcona County) — Carmen's connection. Workshop pricing: $190-$210/person. Based Sherwood Park.
- Lost Lake positioning: premium lakefront art retreat — $300-$500/person weekend
- Carmen Wilde — consider as formal Art Weekend coordinator (admin or on-site). cwildedesigns@shaw.ca
- Willie — local artist, already engaged

Tasks:
— S3.T5.1(au) — Willie: sending Art Weekend ideas in next few days — awaiting
— S3.T5.2(av) — Willie: send website address to Jack for revamp review (no charge offer extended)
— S3.T5.3(aw) — Jack: website revamp option for Willie once URL received
— S3.T5.4(ax) — Scott: consider formalizing Carmen's role in Art Weekend once Willie's ideas arrive
— S3.T5.5(ay) — Kate: monitor ASSC spring schedule for partnership opportunities

S3.T2 — Quote decision
— S3.T2.1(l) — Kate: flag Scott when quote received
— S3.T2.2(m) — Scott: decide insurance vs out of pocket once quote arrives

S3.T3 — Starlink
— S3.T3.1(n) PENDING — Tracking info not yet established. Wait until info is updated.
— S3.T3.2(o) — Kate: track delivery once tracking received

S3.T4 — Router credentials
— S3.T4.1(p) — Scott: get admin credentials from Wayne — wait for appropriate time
— S3.T4.2(q) — Kate: write email to Wayne re router credentials and hold until Scott directs

---

**S4 — Achilles Athletics Inc**

S4.0 — AAI (Company Level) — No active issues
S4.1 — Elite Softball Academy (ESA) — No active issues
S4.2 — Next Level Academy — No active issues

S4.3 — Advice to a Teammate
Issues:
S4.3.I1 — Maeve Lavoy returns St. Albert early May — launch target June 1.

Tasks:
*No active tasks*

---

**S5 — Open Spot Inc**

S5.0 — OSI — No active issues
S5.1 — Save Cash Today — No active issues
Tasks: *No active tasks*

---

**S6 — *(Open — TBD)***
*No active issues or tasks*

---

**S7 — Moon Builder 1037 Inc**

S7.0 — MB (Company Level) — No active issues
S7.1 — Clinic IQ
Issues:
S7.1.I1 — Pricing Intelligence System implementation stalled at RV/TG due to Tammy/Dawn delay (see S1.0.I4)
S7.1.I2 — Dr. Koshy (Rosslyn) — outreach in progress to introduce Pricing Intelligence System

Tasks:
S7.1.T1(as) — Scott: text Dr. Koshy re Pricing Intelligence System — SENT ✅ (no charge offer included)
S7.1.T2(at) — Scott: schedule visit to Rosslyn to walk Dr. Koshy through clinic report
S7.2 — Price Intelligence System — No active issues
S7.3 — 99 Cent Factor — No active issues

S7.4 — AMS-VET
Issues:
S7.4.I1 — AgentMail on free plan — 100 email/day limit.
S7.4.I2 — gog auth (Gmail/Calendar) — incomplete, needs Mac Mini.
S7.4.I3 — Wayne deflecting on Daily codebase access.
S7.4.I4 — MiniMax fallback model not configured.
S7.4.I5 — Microsoft Authenticator broken.
S7.4.I6 — Morning Brief cron erroring on rate limits.
S7.4.I7 — Golias email connection — reply pending.
S7.4.I8 — Sophia (Luke's agent) — welcome sent, reply pending.

Tasks:
S7.T2 — gog auth
— S7.T2.1(t) — Scott: complete gog auth at Mac Mini next visit

S7.T4 — MiniMax fallback
— S7.T4.1(u) — Kate: configure MiniMax as fallback model

S7.T5 — Agent connections
— S7.T5.1(v) — Kate: monitor for Golias and Sophia replies (3003)

---

## Base Camp *(non-revenue)*

**BC.CFL — Collaborative Family Law**
*(details on file — Kate reference)*

Issues:
BC.CFL.I1 — Disclosure package due April 10
BC.CFL.I2 — Prep call Lesley: April 13
BC.CFL.I3 — First CFL meeting: April 14

Tasks:
BC.CFL.T1(I1) — Disclosure package
— BC.CFL.T1.1(w) — Scott: complete monthly budget template
— BC.CFL.T1.2(x) — Scott: complete statement of income, assets & liabilities
— BC.CFL.T1.3(y) — Scott: complete goals worksheet
— BC.CFL.T1.4(z) — Kate: organize house photo documentation for submission

---

**BC.CP — Co-Parenting**

Issues:
BC.CP.I1 — Tournament assignments — all TBD
BC.CP.I2 — Scott to reply to Tammy re: house visit text (Sun Mar 29)

Tasks:
BC.CP.T1(aa) — Scott: review tournament list and assign Scott or Tammy per event
BC.CP.T2(ab) — Scott: reply to Tammy re house visit (Sun Mar 29) ***

---

**BC.K — Kids**

K1 — Jack
Issues:
BC.K.1.I1 — Coach J recruitment letter needed

Tasks:
BC.K.1.T1(ac) — Scott: send Coach J info to Kate for letter draft
BC.K.1.T2(s) — Scott: set up PowerSchool for Jack

K2 — Pete
Issues:
BC.K.2.I1 — BC trip departed Sun Mar 29 ✅
Tasks: *No active tasks*

K3 — Eastynn
Issues:
BC.K.3.I1 — Calgary Sat Mar 28 (Tammy) — filed ✅
BC.K.3.I2 — Scott pickup Mon Mar 30, drop off Thu Apr 2
Tasks: *No active tasks*

K4 — Emmy
Issues:
BC.K.4.I1 — Easton warranty claim — Case #00370604. Online claim still needed.
BC.K.4.I2 — Double header Red Deer today Sun Mar 29 (Scott) 🥎

Tasks:
BC.K.4.T1(ad) — Scott: submit online warranty claim at warranty.rawlings.com (Sun Mar 29)
BC.K.4.T2(r) — Scott: set up PowerSchool for Emmy

---

**BC.S — Family Schedule**

BC.S.1 — Scott: Red Deer double header today 🥎
BC.S.2 — Jack: own schedule
BC.S.3 — Pete: BC trip Sun Mar 29 → Sun Apr 5
BC.S.4 — Eastynn: Mon Mar 30 Scott pickup / Tue Mar 31 riding / Thu Apr 2 riding + drop off
BC.S.5 — Emmy: Sun Mar 29 double header Red Deer / Tue Mar 31 riding (Scott) / Wed Apr 1 return to Tammy / Thu Apr 2 riding (Tammy)
BC.S.6 — Tournament list T1–T10 (see FULL7S V1.21)

---

**BC.H — Health & Well Being**

Issues:
BC.H.I1 — Gretel declining — Scott available if kids need him

Tasks:
BC.H.T1(ae) — Scott: email Tammy about feelings on Gretel (Sun Mar 29)

---

**BC.F — Finances**

Issues:
BC.F.I1 — TD Visa — pay Mon Mar 30 ⚠️
BC.F.I2 — PayPal DVC funds — move to ATB ***
BC.F.I3 — Mastercard payment pending
BC.F.I4 — Rogers $21.23 on cancelled account
BC.F.I5 — CRA account not set up
BC.F.I6 — St. Albert tire expense — receipt needed

Tasks:
BC.F.T1 — TD Visa
— BC.F.T1.1(af) — Scott: pay TD Visa Mon Mar 30 ⚠️

BC.F.T2 — PayPal → ATB → Mastercard ***
— BC.F.T2.1(ag) — Scott: set up banking in PayPal
— BC.F.T2.2(ah) — Scott: transfer DVC funds to ATB
— BC.F.T2.3(ai) — Scott: pay Mastercard from ATB

BC.F.T3 — Rogers
— BC.F.T3.1(aj) — Scott: call 1-877-742-9249 re $21.23

BC.F.T4 — CRA
— BC.F.T4.1(ak) — Scott: set up CRA My Account online

BC.F.T5 — Tire expense
— BC.F.T5.1(al) — Kate: draft letter to City of St. Albert
— BC.F.T5.2(am) — Scott: supply tire receipt to Kate
— BC.F.T5.3(an) — Kate: send draft to scott-wilde@live.com for review
— BC.F.T5.4(ao) — Scott: review and send

---

## UNASSIGNED / TO BE CATEGORIZED

These tasks are in-scope but category/summit TBD.

Tasks:
- UNCATEGORIZED.T1(ap) — Kate: CSI Daily — set up new sections
- UNCATEGORIZED.T2(aq) — Scott: Sirus accounts — figure out which accounts are live and which are not

---

*Code: FULL7S | Version: V1.25 | Locked: March 31, 2026 — Kate*
*Daily task file: 7S-DAILY-2026-03-29.md*
